...
export default function App() {
  ...
  return (
    <div className={`flex flex-col h-screen bg-background font-sans text-foreground selection:bg-blue-900 overflow-hidden relative transition-colors duration-500`}>
            ...
            <div className="space-y-12">
              <AnimatePresence initial={false}>
                {messages.map((msg) => (
                  ...
                  >
                    {msg.role === "user" ? (
                      <div className="glass-card bg-blue-600/90 text-white px-4 py-3 sm:px-6 sm:py-4 rounded-2xl rounded-tr-sm max-w-[90%] sm:max-w-[70%] shadow-lg border-blue-500/50">
                          ...
                          <div className="flex-1 w-full space-y-8 sm:space-y-10 text-left">
                            {msg.details.map((detail, dIdx) => (
                              <div key={`${detail.playerId}-${dIdx}`} className="space-y-6 relative">
                                ...
                                {/* Player Card header */}
                                <div className="flex items-center gap-2 sm:gap-4 glass-card p-3 sm:p-6 border-card-border bg-card-bg/20">
                                    ...
                                    <div className="glass-card p-4 sm:p-6 bg-card-bg/20 hover:bg-card-bg/40 transition-colors shadow-inner border-card-border">
                                      ...
                                      <ul className="space-y-1.5 sm:space-y-3">
                                        {detail.focusPoints.map((point, idx) => (
                                          <li key={idx} className="flex gap-2 sm:gap-3 text-[11px] sm:text-sm text-muted-text items-start">
      ...
      <AnimatePresence>
        {selectedPlayerId && PLAYERS[selectedPlayerId] && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center sm:p-4 bg-background/95 backdrop-blur-xl"
          ...
          >
            <motion.div
              ...
              className="glass-card bg-card-bg shadow-[0_0_100px_rgba(0,0,0,0.5)] w-full max-w-4xl overflow-hidden flex flex-col h-full sm:h-auto sm:max-h-[92vh] border-card-border sm:rounded-3xl rounded-none"
            >
              {/* Modal Header/Hero Area */}
              <div className="relative min-h-[320px] sm:h-80 shrink-0 overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 w-full p-6 sm:p-8 flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-6 bg-gradient-to-t from-card-bg via-card-bg/90 to-transparent">
                  <div 
                  ...
                  <div className="text-center sm:text-left pb-1 flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-center sm:justify-start gap-1 sm:gap-4 mb-1.5 sm:mb-2">
                      <h2 className="text-2xl sm:text-4xl font-black italic tracking-tighter text-foreground drop-shadow-2xl py-1">
                      ...
                      <div className="space-y-3">
                        {PLAYERS[selectedPlayerId].achievements.map((ach, idx) => (
                          <div key={idx} className="flex items-center gap-3 p-4 glass-card bg-gradient-to-r from-amber-500/5 to-transparent border-white/5 hover:border-amber-500/20 transition-colors group/honor">
                            ...
                            <span className="text-sm font-medium text-foreground">{ach}</span>
                          </div>
                        ))}
                      </div>
                    </section>
      ...
      <AnimatePresence>
        {!isInputHidden && (
          <motion.div
            ...
            className="flex-none p-4 w-full max-w-4xl mx-auto absolute bottom-0 left-1/2 -translate-x-1/2 bg-gradient-to-t from-background via-background to-transparent pt-10 z-20"
          >
            {/* Coach Selector */}
